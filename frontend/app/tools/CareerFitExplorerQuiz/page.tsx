...
const CareerQuiz: React.FC = () => {
    ...
    const handleNext = async () => {
        ...
        if (selectedOptionData) {
            ...
            if (currentQuestionIndex < questions.length - 1) {
                ...
                const finalScoreBreakdown = getScoreBreakdownFromAnswers(newAnswers);
                await saveQuizSubmission(finalResult, finalScoreBreakdown, newAnswers);
    ...
    const getScoreBreakdownFromAnswers = (answerList: string[]) => {
        const counts = { A: 0, B: 0, C: 0, D: 0 };
        answerList.forEach((answer) => {
            counts[answer as keyof typeof counts]++;
        });
    ...
    const getScoreBreakdown = () => {
        return getScoreBreakdownFromAnswers(answers);
    };
    ...
    const saveQuizSubmission = async (
        result: Result,
        scoreBreakdown: { A: number; B: number; C: number; D: number },
        answerList: string[],
    ) => {
        ...
        try {
            ...
            const response = await fetch(`${apiBaseUrl}/quiz-submissions`, {
                ...
                body: JSON.stringify({
                    testName: "Career Fit Explorer Quiz",
                    name: trimmedName,
            ...
            <div className="min-h-screen bg-gradient-to-b from-[#FFF5E5] to-[#FFFDF1] py-12">
                <div className="max-w-xl mx-auto px-4">
                    <div className="rounded-2xl border border-[#FFCE99]/30 bg-white p-8 shadow-xl">
                        ...
                        <h1 className="text-2xl font-bold text-[#2F1500] mb-2">
                            Start Your Quiz
                        </h1>
                        <p className="text-sm text-[#7A4A1A] mb-6">
                            Enter your name and phone number before starting. Your quiz result will be saved automatically when you finish.
                        </p>
                        <div className="space-y-4">
                            <input
                                type="text"
                            ...
                            />
                            {saveErrorMessage ? (
                                <p className="text-sm text-red-600">{saveErrorMessage}</p>
                            ) : null}
                            <button
                                type="button"
